<script lang="ts">
  ...
  import { listShare, messengerComponents } from "$lib/utils/config";

  $: $locale && typeof window !== "undefined" && setTimeout(window.Sharer.init);
</script>

{#each listShare[$locale == 'ru' ? $locale : 'default'].map(m => messengerComponents[m]) as messenger (messenger)}
  <div class="d-inline-block px-1" data-sharer={messenger.name} data-title={ browser && $page.url && document.title } data-url={ $page.url }>
    <Button messenger={messenger} height={14} class="p-1 rounded-4"></Button>
  </div>
{/each}
